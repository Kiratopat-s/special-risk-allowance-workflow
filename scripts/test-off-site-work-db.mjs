...
const container = `sraw-osw-test-${randomUUID()}`;
...
try {
  await run("docker", ["run", "--detach", "--rm", "--name", container, "--publish", "127.0.0.1::5432", "--env", `POSTGRES_PASSWORD=${password}`, "--env", "POSTGRES_DB=osw_test", "postgres:17.7"], process.env, true);
  const binding = await run("docker", ["port", container, "5432/tcp"], process.env, true);
  assert.match(binding, /^127\.0\.0\.1:\d+$/);
  const url = `postgresql://postgres:${password}@${binding}/osw_test`;
  let ready = false;
  ...
  const env = { ...process.env, DATABASE_URL: url, OSW_TEST_DATABASE_URL: url, OSW_TEST_CONTAINER: container, NODE_ENV: "test" };
  await run("bun", ["x", "--no-install", "prisma", "migrate", "deploy"], env);
  await run("bun", ["x", "--no-install", "vitest", "run", "--config", "vitest.off-site-work.config.mjs"], env);
} catch (cause) {
  console.error(cause);
  process.exitCode = 1;
} finally {
