...
function Preview() {
  ...
  return <div className="app-public">
    ...
    <nav className="flex flex-wrap gap-2 p-4" aria-label="Fixture scenes">
      {["Roles", "Users", "Departments", "Permissions", "Collections", "Off-site work", "Leader queue", "External verification"].map((name) => <Button key={name} variant={scene === name ? "default" : "outline"} onClick={() => setScene(name)}>{name}</Button>)}
    ...
    <main className="mx-auto max-w-[1600px] p-4" key={scene}>
      ...
      {scene === "External verification" && <div className="mx-auto max-w-2xl"><LeaderVerifyClient token="visual-fixture-token" existingSignatureDataUrl={leaderQueueSignature} /></div>}
    </main>
